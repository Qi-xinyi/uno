...
int menu_select()
{
    ...
    do
    {
        ...
        scanf("%s",s);
        if(strcmp(s,"*")==0) strcpy(s,"12");
        if(strcmp(s,"add*")==0) strcpy(s,"13");
        c=atoi(s);
    }while(c<0||c>13);
    return c;
}
...
int delete(ADDRESS t[], int n) {  
    ...
    if (i > n - 1) { // 如果未找到（索引超出范围）  
        ...
        scanf("%d", &ch); // 输入确认信息  
        if (ch == 1) { // 如果确认删除  
            for (j = i+1; j < n; j++) { // 从当前位置开始，后续记录前移  
...
void addimportant(ADDRESS adr[],int length)
{
    ...
    char a[30];
    scanf("%s",a);
    printf("%d", in_it(adr, a, length) i);
    if (i = in_it(adr, a, length) != 0)
        strcpy(adr[i].name[0], strcat(a, "*"));
    else
    printf("not found");
}